...
<div class="page-title-box d-sm-flex align-items-center justify-content-between">
    ...
    <div class="page-title-right">
        <div class="d-flex justify-content-end gap-2 mb-2">
            <a href="{{ route('laporan.sub_pekerjaan.export', request()->query()) }}" class="btn btn-success btn-sm"><i
                    class="bx bx-file"></i> Export Excel</a>
            <a href="{{ route('laporan.sub_pekerjaan.print', request()->query()) }}" class="btn btn-danger btn-sm"
    ...
    <div class="card-body table-responsive">
        <table class="table table-sm table-hover align-middle">
            ...
            <tbody>
                ...
                : false;
                $isClose = in_array($row->status_progress, ['selesai', 'verifikasi'], true);
                @endphp
                <tr
                    class="{{ $isTerlambat && !$isClose ? 'table-danger' : (in_array($row->status_progress , ['selesai','verifikasi']) ? 'table-success' : 'table-warning') }}">
                    <td></td>
                ...
                <tr>
                    ...
                    <td>{{ number_format($sub->anggaran ?? 0) }}</td>
...
<script>
    ...
    opdSelect.addEventListener('change', function() {
        ...
        if(opdId) {
            fetch(`/laporan/sub-pekerjaan/get-proyek-by-opd/${opdId}`)
                .then(res => res.json())
                .then(data => {
